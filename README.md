---
title: openclash-config-tools
area: project
purpose: guide
lifecycle: active
created: 2026-06-16
updated: 2026-06-19
tags:
  - openclash
  - clash
  - rules
---

# openclash-config-tools

## 核心设计原则

> **IP 稳定 > 一切。宁可慢一点，也不要风控封号。**

| 原则 | 说明 |
|------|------|
| **故转优先** | 所有策略组默认 `type: select` 手动选择固定节点，不自动测速 |
| **住宅 IP 独立** | 手动配置单个节点，不是订阅链接，保证出口唯一 |
| **自动测速降级** | `url-test` 仅作备选，不设为默认，避免频繁切换 IP |
| **AI 默认独立** | 默认固定机场出口；需要时可手动选择链式开关 |
| **分层规则模式** | Layer 2 只带通用规则；个人域名、PT 站等自有规则归入 Layer 3，默认不启用 |

---

## 参考来源

- Mihomo 官方规则说明：`https://wiki.metacubex.one/en/config/rules/`
- Mihomo 官方示例配置：`https://wiki.metacubex.one/en/example/conf/`
- MetaCubeX 规则库：`https://github.com/MetaCubeX/meta-rules-dat`
- blackmatrix7 规则库：`https://github.com/blackmatrix7/ios_rule_script`

---

## 📦 配置文件

| 文件 | 用途 | 你需要改的地方 |
|------|------|---------------|
| `config_local.yaml` | 本地软路由 OpenClash 用 | 填机场订阅链接 + 住宅IP节点信息 |
| `config_mobile.yaml` | 手机 Clash Mi 用（含回家代理） | 填机场订阅链接 + 住宅IP节点信息 + Home的SS密码 |
| `extensions/layer3_extra.yaml` | Layer 3 可选扩展（广告拦截、游戏、加密货币、英伟达、亚马逊、测试） | 按需复制到主配置 |

---

## 🔌 Layer 3 可选扩展

默认配置（Layer 2）只包含通用分流规则。以下高级功能已移入 `extensions/layer3_extra.yaml`，按需启用：

| 类别 | 内容 |
|------|------|
| **广告拦截** | BanAD、Adobe、AdobeActivation 规则集 |
| **游戏平台** | Steam、Epic、EA、Blizzard、Nintend、PlayStation、UBI 规则集 + 「游戏」策略组 |
| **加密货币** | Crypto、Binance、Bybit、OKX 规则集 + 「加密货币」策略组 |
| **英伟达** | Nvidia 规则集 + 「英伟达」策略组 |
| **亚马逊** | Amazon 规则集 |
| **测试** | ChinaTest 规则集 + 「测试」策略组 |

**启用方法**：打开 `extensions/layer3_extra.yaml`，把需要的部分复制到主配置对应位置。

---

## ⚠️ 使用前只需要填 3-4 个地方！

### 👉 机场订阅（两个配置都要填）

```yaml
proxy-providers:
  聚合机场:
    url: "https://你的机场订阅链接"  # ✅ 改这里
```

### 👉 住宅 IP 节点（两个配置都要填，手动配置，不是订阅！）

```yaml
proxies:
  - name: 住宅-socks5
    type: socks5
    server: "你的住宅IP地址"  # ✅ 改这里
    port: 12345                # ✅ 改这里
    username: "你的用户名"      # ✅ 改这里
    password: "你的密码"        # ✅ 改这里
    dialer-proxy: 住宅中继组    # 经独立机场选择组中转
```

**重要：住宅 IP 不是订阅链接！不要用 proxy-provider 拉取。**

### 👉 仅手机版：回家节点密码

```yaml
proxies:
  - name: 回家
    password: "你的SS密码"  # ✅ 改这里（回家代理的密码）
```

---

## ✅ 设计特点

1. **IP 稳定优先**：默认走「故转」固定节点选择，不自动测速，不频繁切换 IP
2. **住宅 IP 手动配置**：单个节点手动填，不是订阅，出口唯一
3. **几百上千条通用规则全靠社区自动维护**：通过 rule-provider 自动从 meta-rules-dat/blackmatrix7 更新
4. **自有规则默认不加载**：个人域名、M-Team 等保存在 Layer 3，需要时再复制内联
5. **按地区过滤节点**：自动从订阅里过滤「新加坡/香港/日本/美国」节点，按名字匹配
6. **AI 与 Google 独立增强**：本地版在通用模板基础上额外提供住宅 SOCKS5
7. **相同服务商在同一规则集**，避免同一个服务商的不同域名走不同节点导致 IP 漂移
8. **业务组选项统一**：普通业务只保留通用、全量故转/手动/自动和直连；地区细分集中在通用选择
9. **手机版独有回家开关 + `*.l-home.top` 直连规则**，避免回环

> `rules/MyDirect.list` 是 Layer 3 可选规则源。由于本仓库为私有仓库，不能把其 raw URL 直接作为无认证 rule-provider；启用时应把需要的条目复制到主配置 `rules` 顶部。

---

## 策略组说明（用户视角）

你只需要关心两个地方，其他都不用改：

1. **「通用选择」** —— 日常只用改这个，选你想用的地区节点
2. **某个业务策略组需要单独切节点** —— 比如 Netflix 必须走新加坡，就单独把流媒体切到新加坡

**默认路径**：所有流量 → 通用选择 → 你选的地区节点

---

## 全局原则

1. **普通业务统一使用 default 模板**，切换「通用选择」即可改变默认出口。
2. **相同服务商必须在同一规则内**，避免同一个服务商的不同域名走不同节点导致 IP 漂移。
3. **本地版通用选择提供住宅 SOCKS5**；人工智能和谷歌额外把它放在一级选项，方便快速切换。
4. **住宅中继不使用美国或未经验证的自动组**；当前仅保留台湾、日本、其他三个通过隔离测试的地区自动组。
5. **手机版与家用版使用同一套通用和住宅策略**，只额外增加 `回家开关`：默认进入通用选择，需要时切到回家。

---

## 绝对不要做的事

❌ **不要把任何策略组默认设为 url-test 自动测速** —— 5 分钟切一次 IP，任何网站都能看出来你是代理

❌ **不要用 proxy-provider 拉取住宅 IP** —— 住宅 IP 就是一条线路，不是节点池

❌ **不要频繁切换 AI 的普通/住宅出口** —— 可以手动使用链式，但应长期保持同一出口

---

## 最佳实践

1. **先手动选好一个节点**，选好了就别动了，用一个月再说
2. **不要没事就切节点**，IP 越稳定，风控概率越低
3. **住宅 IP 只在必要时临时打开**，用完就关，不要一直开着
4. **AI 服务就一直用同一个地区的同一个节点**，不要换

---

## 发布前验证

```bash
python3 validate_config.py
```

验证脚本覆盖 YAML、引用、敏感字段、策略循环、唯一 MATCH、两端 Layer 2 数量与私网路由差异、Layer 3 扩展完整性，以及 provider URL 可访问性。

**验证失败不能发布！**

### 隔离验证主路由候选配置

当配置会影响当前在线网络时，不要直接在主路由上试错。当前项目已经有一套隔离验证路径：它会读取项目根目录的 `PRIVATE-CONNECTION-INFO.md`，拉取路由器当前 Mihomo 二进制与已缓存的 provider 文件，在本机 `Ubuntu WSL` 中启动一个只监听 `127.0.0.1` 的临时运行时，再自动验证控制面、策略组切换和真实出站。

前置条件：

1. Windows 已安装并可运行 `WSL`
2. 可从当前机器 SSH 到 OpenClash 路由器
3. 已设置路由器密码环境变量 `OC_ROUTER_PASSWORD`

命令（在 `E:\SynologyDrive\OrbitOS` 根目录执行）：

```bash
python .orbitos/tmp/validate_openclash_isolated_runtime.py --profile local
python .orbitos/tmp/validate_openclash_isolated_runtime.py --profile mobile
```

成功条件：

1. 同版本 Mihomo 能正常启动并加载 provider
2. 至少有一个 `所有-手动` 候选节点能通过 `http://neverssl.com` 与 `https://www.gstatic.com/generate_204`
3. `https://ip.net.coffee/` 通过混合代理访问成功

这一步只证明“候选配置在隔离环境可工作”。主路由上线前，仍应再用 Net.Coffee 的 IP / DNS / WebRTC 页面复验运行时结果。

---

## 更新日志

### 2026-06-19 Architecture v2.0 分层架构落地

- ✅ Layer 2 基线裁剪完成：主配置规则集 42 → 25，家用版 31 个策略组、手机版 32 个策略组
- ✅ Layer 3 可选扩展：广告拦截、游戏、加密货币、英伟达、亚马逊、测试全部移入 `extensions/layer3_extra.yaml`
- ✅ 分层原则：Layer 2 只保留通用必需功能；Layer 3 高级功能按需启用
- ✅ 验证脚本更新：跨配置一致性检查 + 敏感字段检测
- ✅ 两份配置 14 项检查全部通过

### 2026-06-19 性能优化

- 🔥 删除 7 个重复冗余的规则集
- ⚡ 10 个规则集从 list 纯文本升级为 mrs 二进制格式，加载速度大幅提升
- ✅ 完善发布前验证脚本，9 项检查不通过不能发布

### 2026-06-19 配置对齐

- ✅ 手机版与家用版规则集完全对齐，新增 25 个缺失规则集
- ✅ 修复策略组引用顺序问题
- ✅ 修复规则引用格式：添加 `/ Domain` 后缀
- ✅ 删除自创策略组，规则引用改为已有策略组
- ✅ 完善验证脚本：增加锚点格式检测 + pre-commit hook
- ✅ 清理一次性脚本，只保留发布验证脚本

### 2026-06-18 重大修正

- ✅ 住宅 IP 改为手动配置在 `proxies` 下，不再是 proxy-provider 订阅
- ✅ 所有策略组默认走「故转」固定节点选择模式
- ✅ `url-test` 自动测速降级为备选，不在默认选项前列
- ✅ 手机配置补上缺失的「故转」地区分组
- ✅ 所有文档对齐最新状态
